...
def checkBoards(boards: List[Board]) -> Board:
  hasWinningBoard = False;
  winningBoard = None;

  # for board in boards:
  boardIdx = 0;
  while hasWinningBoard == False and boardIdx < len(boards):
...
def getWinner(bingo: Bingo) -> Winner:
  ...
  drawnIdx = 0;
  while winner == None and drawnIdx < len(drawnNumbers):
    drawnNumber = drawnNumbers[drawnIdx];
    bingo.boards = markBoards(drawnNumber, memo, boards);
    winningBoard = checkBoards(bingo.boards);
    if winningBoard is not None:
      winner = Winner(winningBoard, drawnNumber);

    drawnIdx += 1;
  
  return winner;

def calculateScore(winner: Winner):
  tiles = winner.board.tiles;
  markedTiles = winner.board.markedTiles;
  nRow = len(tiles);
  nCol = len(tiles[0]);
  ...
  return unmarkedScore * int(winner.drawnNumber);
